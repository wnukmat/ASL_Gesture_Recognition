...
class training_set():
	'''
	Loads training set, creates class attributes for ease of use in batch training
	'''
	...
	def _augment(self, sample):
		examples = [] 
		for i in range(0, sample.shape[0] - self.winsize, 5):
			...
			for features in os.listdir(subdir[0]):	
				...
				if features.endswith(".npy"):						
					...
					for j in range (augmented_samples.shape[0]):
						...
						if(int(np.random.uniform(1,11)) % 10 == 0):
							self.validation_samples.append(list(np.squeeze(augmented_samples[j,:,:])))
	...
	def save_set(self):
		...
		np.save('validation_samples', self.validation_samples)
		np.save('validation_labels', self.validation_labels)
	...
	def reload_set(self):
		...
		self.validation_samples = np.load('validation_samples.npy')
		self.validation_labels = np.load('validation_labels.npy')
	...
	def validation_batch(self):
		return self.validation_samples, self.validation_labels
